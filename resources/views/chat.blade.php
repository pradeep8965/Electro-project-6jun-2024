@extends('layouts.app')

@section('main')
    <main id="content" role="main" class="container-fluid mt-5 mb-5">
        <div class="card text-center ">
            <div class="card-header bg-light text-left container">
            <img src="data:image/svg+xml;base64," />
                <div class="margin-left" style="font-weight: bold; font-size: 30px;">Flipkart Support</div>
            </div>
            <div class="card-body" style="min-height:350px;">
                <div id="messages">
                    @foreach ($messages as $message)
                        <div class="message">
                            {{ $message->content }}
                        </div>
                    @endforeach
                </div>
            </div>
            <div class="card-footer text-body-secondary bg-white">
                <form method="POST" action="{{ route('chat.index') }}">
                    @csrf
                    <div class="mb-3">
                        <div class="row">
                            <div class="col-10">
                                <input type="text" class="form-control-lg w-100" name="content" placeholder="Write a Message...">
                            </div>
                            <div class="col-2">
                                <button type="submit" class="btn btn-primary btn-lg w-100">
                                    <i class="fa-solid fa-paper-plane"></i>
                                </button>
                            </div>
                        </div>
                    </div>
                </form>
            </div>
        </div>
    </main>
@endsection
                        